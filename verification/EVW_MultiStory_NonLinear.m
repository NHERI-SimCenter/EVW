...
P= P*1.0;% scaling
dt = 0.02;
check = @(dU,dR)sum(abs(dU))<1e-1
for j=1:numStory
    myspring{j}=ElasticPPSpring(k,30);
end
gamma = 0.5;
beta=0.25;
[u,v,a,fs]=NonlinearNewmark_multiStory(numStory,M,C,zeros(numStory,1),zeros(numStory,1),dt,P,gamma,beta, ...
			    myspring,'Newton',50,check);
...
dtw = 0.1;
check = @(dU,dR)sum(abs(dR))<1e-1
for j=1:numStory
    myspring{j}=ElasticPPSpring(k,30);
end
gamma = 0.5;
beta=0.25;
[uw,vw,aw,fsw]=NonlinearNewmark_multiStory(numStory,M,C,zeros(numStory,1),zeros(numStory,1),dtw,Pw,gamma,beta, ...
			    myspring,'Newton',50,check);
